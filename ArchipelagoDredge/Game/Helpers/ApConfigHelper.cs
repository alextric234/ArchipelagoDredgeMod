...
namespace ArchipelagoDredge.Game.Helpers
{
    public static class ApConfigHelper
    {
        ...
        public static (string host, int port, string slot, string pwd) Read()
        {
            ...
            string host = cfg["apIpAddress"]?["value"]?.ToString() ?? "";
            int port = (int?)cfg["apPort"]?["value"] ?? 38281;
            string slot = cfg["apSlotName"]?["value"]?.ToString() ?? "";
            string pwd = cfg["apPassword"]?["value"]?.ToString() ?? "";
